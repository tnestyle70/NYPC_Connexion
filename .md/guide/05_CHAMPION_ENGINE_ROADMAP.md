...
>
> **→ 재검증으로 위 정정을 기각 (2026-06-11, 동일 로그 플레이어별 획득 센서스):**
> FIRST·SECOND 각각 정확히 16타입 × 2장 = 32장 획득, 3장 이상 보유 타입 0건.
> 공용 54장 주머니라면 양쪽 모두 전 타입 2-2 분할일 확률 ≈ 0.375¹⁶ ≈ 1.5×10⁻⁷.
> 결론: **공식 규칙 그대로 각자 주머니 16종×2장 = 32장** (전역 합 64장·잔량 합 54장은 양 모델 공통이라
> 판별력 없음). 구현·referee는 각자-주머니 모델을 따른다. 검증 도구: `scripts/shadow_replay.py` 센서스 라인.
